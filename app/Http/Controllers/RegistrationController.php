...
class RegistrationController extends Controller
{
    ...
    /**
     * Display the edit form for a specific registration.
     *
     * @param int $id
     * @return \Illuminate\View\View
     */
    public function edit($id)
    {
        ...
        // Prepare documents JSON
            $documentsJson = $registration ? json_encode($registration->documents->map(function ($doc) {
                return [
                    'type' => $doc->type,
                    'path' => $doc->path,
                    'url' => $doc->url,
        ...
        return view('registration.edit-form', compact('registration', 'states', 'districts', 'optionalSubjects', 'documentsJson'));
    }

    /**
     * Update the registration form data and generate PDF.
     *
     * @param \Illuminate\Http\Request $request
     * @param int $id
     * @return \Illuminate\Http\JsonResponse
    ...
    public function update(Request $request, $id)
    {
        ...
        $timestamp = Carbon::now()->format('YmdHis');
        $updatedDocuments = [];
        $existingDocs = $registration->documents->pluck('path')->toArray();
        ...
        foreach ($documents as &$doc) {
            ...
            if (!in_array($doc['path'], $existingDocs)) {
                $docType = str_replace(' ', '_', $doc['type']);
                $originalName = pathinfo($doc['name'], PATHINFO_FILENAME);
                $sanitizedName = preg_replace('/[^a-zA-Z0-9_-]/', '_', $originalName);
                $extension = pathinfo($doc['path'], PATHINFO_EXTENSION);
